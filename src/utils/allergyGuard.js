// src/utils/allergyGuard.js
//
// Shared allergy-safety layer for the AI Clinical Consult feature.
// This is a direct port of NACON-EMR's src/lib/allergyGuard.js — kept in
// sync manually since the two apps are separate codebases, but the logic
// itself is intentionally simple, deterministic string/class matching (not
// an AI call) so it can't be talked out of flagging something, and both
// apps should behave identically here.
//
// Used in two places:
//   1. clinicalPlan.js       — BEFORE the prompt is sent, to strip any
//      MedIndex-matched drug that conflicts with a recorded allergy out of
//      what the model is even offered, and to turn the allergy list into an
//      explicit hard-constraint instruction.
//   2. AiClinicalConsultPanel.jsx — AFTER the model responds, to re-check
//      every drug name it actually suggested (belt-and-suspenders: never
//      trust the model alone to have honored the instruction).

// Known cross-reactive drug classes. Keyed by a normalized allergy term a
// user might type; value is a list of generic names / class fragments that
// should also be treated as allergic even if not an exact name match. Not
// exhaustive — extend as needed — but covers the common clinically
// significant cross-reactivities.
...
// offer to the model and the excluded list (kept for the "excluded due to
