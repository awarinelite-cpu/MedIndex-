...
function AiClassFallback({ className, existingDrugs }) {
  ...
  const saveAllToDatabase = async (items) => {
    ...
    // Save EVERY drug the AI listed — no skipping. If a drug with the same
    // name already exists it simply gets replaced. Duplicate cleanup will be
    // handled later by the admin duplicate detector.
