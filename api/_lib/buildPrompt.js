...
export function buildPrompt(body) {
  const {
    mode = 'drug',
    genericName, brandNames, drugClass, knownData, notInDatabase, externalContext,
  ...
  if (mode === 'class') {
    ...
    return {
      maxTokens: 1800,
      prompt: `You are assisting a licensed nurse using a clinical drug reference app in Nigeria. List medications (generic names) that belong to the drug class "${className}" or its recognized subclasses.\n${knownList}\nInclude first-line and commonly used second-line agents. Do not include a medication whose primary pharmacological class is something else, even if occasionally used off-label for a related condition.\n\nGroup them by subclass using ## markdown headers where subclasses exist. For each medication, use a bullet point starting with the **generic name in bold**, followed by: primary indication, typical route, and any notable distinguishing feature. Keep each note to one short line.\n\nList roughly 12-20 medications — the most clinically important ones for this class, not an exhaustive catalog. This is reference material only.`,
    };
  }
  ...
  // Live grounding pulled from openFDA/RxNorm just before this prompt was
  // built (see api/_lib/externalDrugSources.js). Treated as authoritative
  // over training-data recall since it comes straight from the FDA label
  // and NIH's drug nomenclature service, not the model's memory.
