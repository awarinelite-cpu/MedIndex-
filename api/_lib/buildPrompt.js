...
export function buildPrompt(body) {
  ...
  if (mode === 'class') {
    ...
    return {
      maxTokens: 3000,
      prompt: `You are assisting a licensed nurse using a clinical drug reference app in Nigeria. Search hard and exhaustively for medications (generic names) that belong to the drug class "${className}" or its recognized subclasses — this is a systematic database-building exercise, not a quick top-of-mind list.\n${knownList}\nInclude first-line AND second-line agents, older/generic agents still in clinical use, and less commonly prescribed members of the class alongside the well-known ones — not just the most popular few. Do not include a medication whose primary pharmacological class is something else entirely, even if it is occasionally used off-label for a condition associated with "${className}" — every medication listed must genuinely belong to "${className}" or one of its recognized subclasses.\n\nGroup them by subclass using ## markdown headers where subclasses exist. For each medication, use a bullet point starting with the **generic name in bold**, followed by: primary indication, typical route, and any notable distinguishing feature.\n\nAim for as complete a list as clinically justified — typically 15-40 medications for a broad class, fewer for a narrow one. Do not pad with duplicates or near-identical salt forms just to hit a number. This is reference material only.`,
    };
  }
